...
## Introduction
L’objectif de cette présentation est d’expliquer le fonctionnement de la méthode Kanban dans un contexte réel d’équipe projet[cite: 17], puis de montrer comment cette approche a évolué vers un modèle hybride appelé Scrumban.
Nous analyserons l’origine de la méthode [cite: 22], ses principes [cite: 25], son fonctionnement concret [cite: 31], ses forces et ses limites [cite: 39], avant d’établir une comparaison structurée avec Scrumban[cite: 42].
...
La méthode Kanban trouve son origine dans les années 1950 au sein de l’entreprise Toyota[cite: 23]. Elle s’inscrit dans la philosophie du Lean Manufacturing, visant à optimiser les flux de production et à réduire les gaspillages.
...
Kanban répond principalement aux problématiques suivantes[cite: 24]:
...
Kanban repose sur six pratiques principales[cite: 26]:
...
La logique centrale est une logique de flux continu fondée sur un système « pull » (flux tiré) : une tâche n’entre dans le processus que lorsqu’il y a de la capacité disponible[cite: 32].
...
L’outil central est le tableau Kanban[cite: 35]. Contrairement aux idées reçues, il ne se limite pas à "À faire / En cours / Terminé". Pour refléter la réalité du processus, les étapes sont plus détaillées :
* **Backlog :** Le réservoir de toutes les demandes[cite: 35]. 
...
*(Ajouter un visuel : Image d'un tableau Kanban détaillé avec les colonnes Backlog, To Do, Dev, Review, Done)* [cite: 45]
...
Le WIP représente l'ensemble du travail en cours[cite: 37]. Son but principal est de **garantir la fluidité** et d'éviter les goulots d'étranglement.
...
* **Conséquence :** Si la limite est atteinte, l'équipe ne commence pas de nouvelle tâche. Les membres doivent s'entraider pour terminer ce qui est en cours, favorisant ainsi l'intelligence collective[cite: 30, 32].
...
* **Lead Time :** temps total entre la demande et la livraison[cite: 37].
...
*(Ajouter un visuel : Graphiques comparatifs de débit)* [cite: 45]

### c) Exemple d’application sur 2 semaines
*Ici, on illustre la vie d'une équipe sur une courte période*[cite: 38, 39].
...
### Forces
* **Réactivité et Agilité :** Adaptation continue aux changements sans attendre la fin d'un cycle[cite: 27].
* Mise en place sans rupture (on part de l'existant).
* Idéal pour la maintenance, le support et les flux continus[cite: 40].
* Rend les blocages immédiatement visibles (grâce au WIP).

### Limites et Dérives 
* **Moins structurant que Scrum :** Pas de réunions imposées (planning, rétro), ce qui demande une **grande maturité et discipline** de l'équipe pour ne pas tomber dans l'anarchie[cite: 41].
...
* **De Scrum, il garde :** les rôles définis (PO, Scrum Master), un backlog priorisé, les cérémonies régulières (Daily, Rétrospective), et souvent une notion d'itération (planning à court terme)[cite: 29, 32].
...
*(Ce tableau répond à la consigne de comparer avec Scrum)* [cite: 42]
...
* **Kanban est particulièrement adapté :** équipes de support, gestion/correction de bugs de production, flux de demandes imprévisibles, environnements très changeants[cite: 40].
